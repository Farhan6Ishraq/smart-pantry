...
const DotField = memo(({
  ...
  const propsRef = useRef<DotFieldConfig>({
    dotRadius,
    dotSpacing,
    cursorRadius,
    cursorForce,
  ...
  useEffect(() => {
    ...
    function buildDots(w: number, h: number) {
      ...
      const padX = (w % step) / 2;
      const padY = (h % step) / 2;
      const dots: DotState[] = new Array(rows * cols);
      let idx = 0;
    ...
    function tick() {
      ...
      const inBounds = m.x >= 0 && m.x <= w && m.y >= 0 && m.y <= h;
      const baseEngagement = inBounds ? 0.28 : 0;
      const targetEngagement = Math.max(baseEngagement, Math.min(m.speed / 5, 1));
      ...
      if (glowEl) {
        ...
        glowEl.style.opacity = `${Math.min(glowOpacity.current * 0.75, 0.55)}`;
      }
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (distSq < crSq && eng > 0.01) {
          ...
          if (isBulge) {
            ...
            const angle = Math.atan2(dy, dx);
            const move = (500 / Math.max(dist, 0.001)) * (m.speed * p.cursorForce);
